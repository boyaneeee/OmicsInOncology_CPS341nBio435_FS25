...
SuppS6 <- read_excel("C:/Users/Admin/Downloads/science.abf3066_tables_s2_to_s12/science.abf3066_Table_S6.xlsx", sheet="Differential Interaction Score")
...
```{r}
interactionGraph <- makeproteingraph(baits, preys, weight=1)
```
...
renderGraph(layoutGraph(interactionGraph, layoutType="neato"))
```
...
```{r}
baits_sub <- SuppS6_sub$Bait
preys_sub <- SuppS6_sub$Prey
ppiGraph <- makeproteingraph(baits_sub, preys_sub, weight = 1)
```

3. layout the ppiGraph using the layoutGraph(.., layoutType="neato") function

```{r}
layoutGraph(ppiGraph, layoutType="neato")
```

4. render the graph.

```{r}
renderGraph(layoutGraph(ppiGraph, layoutType="neato"))
```
...
node_counts <- degree(ppiGraph)

# Use table to count frequencies of each degree value
degree_frequencies <- table(node_counts)
...
```{r}
connected_components <- connectedComp(ppiGraph)
